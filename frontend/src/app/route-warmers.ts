...
import { learningQueryKeys } from '../entities/student/queries/learning.queries';
...
async function warmStudentProfileRoute() {
  const currentUser = await warmCurrentUser();
...
async function warmStudentCoursesEntryRoute() {
  await Promise.all([
    ...
    warmQuery({
      queryKey: learningQueryKeys.schedulePublishStatus(DEFAULT_SEASON_ID),
      queryFn: () => getSchedulePublishStatus(DEFAULT_SEASON_ID),
    }),
    warmQuery({
      queryKey: learningQueryKeys.myScheduleEvents(DEFAULT_SEASON_ID),
      queryFn: () => getMyScheduleEvents(DEFAULT_SEASON_ID),
    }),
  ]);
}
